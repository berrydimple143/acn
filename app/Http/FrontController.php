...
class FrontController extends Controller {
    ...
    public function checkEmail($email) {
		try {
			$user = User::where('customer_email', $email)->firstOrFail();
			$go = "found";               
			...
			if($hasEmail == "found") {
				return view('site.email_exist', ['title' => 'Email already exist.', 'firstname' => $firstname, 'lastname' => $lastname, 'email' => $email, 'mobile' => $mobile, 'runningbg' => session('runningbg')]);
					...
					$data = [
					 ...
					 return view('site.code_successful', ['title' => 'Code Successful Page', 'runningbg' => session('runningbg')]);
			}
		} catch(\Exception $e) {
		...
		try {
			...
			if($code == $right_code) {				
				try {
					...
					if($this->getMobileLength($mobile) == "0" || $this->getMobileLength($mobile) == 0 || strlen($mobile) == 9) {
							$mobile2 = '0'.substr($mobile, -9);
					}
					$user = User::where('customer_mobile', $mobile)
							->orWhere('customer_mobile', $mobile2)
							->firstOrFail();
					$data = [
					...
					$hasEmail = $this->checkEmail($email);
					if($hasEmail == "found") {
						$usr = User::where('customer_email', $email)->first();
						session(['user' => $usr]);
						session(['loggedin' => 'yes']);
						return redirect()->route('home');
					} else {
